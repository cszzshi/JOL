...
class AnchorHead(BaseDenseHead):
    ...
    def _get_targets_single(self,
        ...
        if len(pos_inds) > 0:
            ...
            # for i in pos_inds:
            #     count = count + 1
            #     gt_bbox = gt_bboxes[assigned_gt_inds[i] - 1]
    ...
    def cls_match(self, cls_score, labels):
        ...
        nll_loss = 1 - log_probs[range(cls_score.shape[0]), labels - 1]
        return nll_loss
    ...
    def loss_by_feat_single(self, cls_score: Tensor, bbox_pred: Tensor,
        ...
        anchors = anchors.reshape(-1, anchors.size(-1))
        bbox_pred_xyxy = self.bbox_coder.decode(anchors, bbox_pred)
        bbox_pred_xyxy = get_box_tensor(bbox_pred_xyxy) * bbox_weights
        bbox_targets_xyxy = self.bbox_coder.decode(anchors, bbox_targets)
        bbox_targets_xyxy = get_box_tensor(bbox_targets_xyxy) * bbox_weights
        ...
        alpha = 1
        d = 2 - (d_loc * d_cls) ** alpha
        d = d * bbox_weights_single

        d_4 = d.unsqueeze(1)
        d_4 = d_4.expand(-1, 4)
        bbox_weights = bbox_weights * d_4

        # ignore and negative
        ignore_mask = label_weights == 0
        positive_mask = bbox_weights_single == 1
        negative_mask = (bbox_weights_single == 0) * (ignore_mask != 1)
        label_weights = d * positive_mask + negative_mask
