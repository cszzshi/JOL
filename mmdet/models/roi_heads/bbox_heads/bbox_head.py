...
class BBoxHead(BaseModule):
    ...
    def iou(self, bbox_pred_xyxy, bbox_targets_xyxy):
        x1_p = bbox_pred_xyxy[:, 0]
        y1_p = bbox_pred_xyxy[:, 1]
        x2_p = bbox_pred_xyxy[:, 2]
        y2_p = bbox_pred_xyxy[:, 3]
        x1_t = bbox_targets_xyxy[:, 0]
        y1_t = bbox_targets_xyxy[:, 1]
        x2_t = bbox_targets_xyxy[:, 2]
        y2_t = bbox_targets_xyxy[:, 3]
        area_p = (x2_p - x1_p) * (y2_p - y1_p) + 1e-6
        area_t = (x2_t - x1_t) * (y2_t - y1_t) + 1e-6
        x1_u = torch.max(x1_p, x1_t)
        y1_u = torch.max(y1_p, y1_t)
        x2_u = torch.min(x2_p, x2_t)
        y2_u = torch.min(y2_p, y2_t)
        area_u = (x2_u - x1_u) * (y2_u - y1_u)
        iou = area_u / (area_p + area_t - area_u)
        iou = torch.clamp(iou, min=0, max=1)
        return iou

    def cls_match(self, cls_score, labels):
        log_probs = -F.log_softmax(cls_score, dim=1)
        log_probs = log_probs / \
                    torch.sum(log_probs, dim=1).unsqueeze(1).expand(-1, log_probs.size(1))
        nll_loss = 1 - log_probs[range(cls_score.shape[0]), labels]
        return nll_loss

    def get_areas(self, bbox_targets_xyxy):
        x1_t = bbox_targets_xyxy[:, 0]
        y1_t = bbox_targets_xyxy[:, 1]
        x2_t = bbox_targets_xyxy[:, 2]
        y2_t = bbox_targets_xyxy[:, 3]
    ...
    def loss(self,
        ...
        # ours
        bbox_weights_single = bbox_weights[:, 0]
        ...
        if bbox_pred is not None:
            ...
            bbox_weights_pos = bbox_weights[pos_inds.type(torch.bool)]
            # do not perform bounding box regression for BG anymore.
            bbox_pred_xyxy = self.bbox_coder.decode(rois[:, 1:], bbox_pred)
            bbox_pred_xyxy = get_box_tensor(bbox_pred_xyxy).view(
                bbox_pred.size(0),
                -1)
            # bbox_pred_xyxy = get_box_tensor(bbox_pred_xyxy).view(
            #     bbox_pred.size(0), self.num_classes,
            #     -1)
            # labels_ = torch.minimum(labels, torch.tensor(self.num_classes - 1))
            # labels_ = labels_.view(labels.shape[0], 1, 1)
            # bbox_pred_xyxy = bbox_pred_xyxy.gather(1, labels_.expand(-1, 1, 4))
            # bbox_pred_xyxy = bbox_pred_xyxy.squeeze(1)
            bbox_targets_xyxy = self.bbox_coder.decode(rois[:, 1:], bbox_targets)
            bbox_targets_xyxy = get_box_tensor(bbox_targets_xyxy)
            d_loc = self.iou(bbox_pred_xyxy, bbox_targets_xyxy)
            # d_loc_pos = d_loc[pos_inds.type(torch.bool)]

            d_cls = self.cls_match(cls_score, labels)
            # d_cls_pos = d_cls[pos_inds.type(torch.bool)]

            # areas
            areas_bbox_targets = self.get_areas(bbox_targets_xyxy)
            areas_bbox_targets_pos = areas_bbox_targets[pos_inds.type(torch.bool)]
            avg_area_bbox_targets_pos = torch.mean(areas_bbox_targets_pos)
            delta = 0.5
            if areas_bbox_targets_pos.numel() > 0:
                # d_a = delta * (1 + torch.exp(1 - (areas_bbox_targets /
                #                                   avg_area_bbox_targets_pos)))
                ...
                d_4 = d.unsqueeze(1)
                d_4 = d_4.expand(-1, 4)
                d_4_pos = d_4[pos_inds.type(torch.bool)]
                bbox_weights_pos = bbox_weights_pos * d_4_pos
            if pos_inds.any():
        ...
        ignore_mask = label_weights == 0
        positive_mask = bbox_weights_single == 1
        negative_mask = (bbox_weights_single == 0) * (ignore_mask != 1)
        # negative_weight = max(1, torch.sum(positive_mask).item()) / torch.sum(negative_mask)
        if torch.sum(positive_mask) > 0:
            d_neg = torch.sum(d * positive_mask) / torch.sum(positive_mask)
        else:
            d_neg = 1
        label_weights = d * positive_mask + negative_mask * d_neg
